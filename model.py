...
def stage4(x,is_training):
    x = bottleneck(x, 16,is_training, downsampe=True)
    x = bottleneck(x, 16,is_training)
    x = bottleneck(x, 16,is_training)
    x = bottleneck(x, 16,is_training)
    return x

def stage0(x,is_training):
    x = bottleneck(x, 32,is_training, downsampe=True)
    x = bottleneck(x, 32,is_training)
    x = bottleneck(x, 32,is_training)
    x = bottleneck(x, 32,is_training)
    return x
...
def spatial_pooling(input):
    ...
    p1 = tf.image.resize_bilinear(tf.layers.average_pooling2d(input, 2, 2), (h, w))
    p2 = tf.image.resize_bilinear(tf.layers.average_pooling2d(input, 4, 4), (h, w))
    p3 = tf.image.resize_bilinear(tf.layers.average_pooling2d(input, 8, 8), (h, w))
    # p4 = tf.image.resize_bilinear(tf.layers.average_pooling2d(input, 8, 8), (h, w))
...
def mapnet(input, is_training=True):
    channels_s2 = [32, 64]
    channels_s3 = [32, 64, 64]
    num_modules_s2 = 2
    num_modules_s3 = 3

    # conv_1 = conv2d(input[:,:,:,0:3], 64, stride=2)
    conv_1 = conv2d(input, 32, stride=2)
    conv_c4 = conv2d(input[:, :, :, 3:4], 32, stride=2)
    conv_c4 = stage4(conv_c4, is_training)
    # conv_d4 = tf.layers.max_pooling2d(conv_c4, 2, 2)
    # conv_d4 = stage4(conv_d4, is_training)
    # print(conv_1)
    # print(conv_c4)

    conv_1 = bn(conv_1, is_training)
    conv_1 = tf.nn.relu(conv_1)
    conv_2 = conv2d(conv_1, 32)
    conv_2 = bn(conv_2, is_training)
    conv_2 = tf.nn.relu(conv_2)
    conv_3 = conv2d(conv_2, 32)
    conv_3 = bn(conv_3, is_training)
    conv_3 = tf.nn.relu(conv_3)
    # conv_4 = tf.layers.max_pooling2d(conv_3, 2, 2)

    stage1 = stage0(conv_3,is_training)
    trans1 = translayer([stage1], [128], channels_s2,is_training)
    ...
    print(stage3)
    # stg3=tf.concat([stage3[0],stage3[1],stage3[2],conv_d4],axis=-1)
    stg3 = tf.concat([stage3[0], stage3[1], stage3[2]], axis=-1)
    squeeze=channel_squeeze(stg3, stg3.shape[-1], name="squeeze")

    # psp spatial
    # spatial=tf.concat([stage3[0],stage3[1]],axis=-1)
    # spatial=spatial_pooling(stage3[1])
    # spatial = pyramid_pooling_block(stage3[1], [2,4,8])
    # new_feature = tf.concat([spatial, squeeze], axis=-1)
    # new_feature = bn(new_feature, is_training)

    new_feature = bn(squeeze, is_training)

    new_feature = tf.nn.relu(new_feature)
    result=conv2d(new_feature, 64, 1, padding='SAME')

    up1=tf.image.resize_bilinear(result,size=(stage3[0].shape[1]*2,stage3[0].shape[2]*2))
    # up1 = bn(tf.concat([up1,conv_c4],axis=-1), is_training)
    ...
    up2 = bn(up1, is_training)
    up2 = tf.nn.relu(up2)
    final = conv2d(up2, 1, 1, padding='valid')


    ed2 = tf.image.resize_bilinear(tf.add(result,conv_c4), size=(result.shape[1] * 2, result.shape[2] * 2))
    ed2 = bn(ed2, is_training)
    ed2 = tf.nn.relu(ed2)
    ed2 = conv2d(ed2, 16, 3)
